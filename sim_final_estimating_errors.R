...
for (i in 1:NUMSAMP) {
  ...
  err_moving <- boot_movingcircle_lm(df_train, RADIUS, NBOOT, model_formula=model_formula)
  
  # Output results
  err_df <- rbind(err_df, c(i, err_true, err_train, err_cv, err_LOO, err_grid_buffer, err_SLOO, err_moving))
}
colnames(err_df) <- c("set_number", "err_true", "err_train", 
                      "err_cv", "err_LOO", "err_grid_buffer", "err_SLOO", "err_moving")
